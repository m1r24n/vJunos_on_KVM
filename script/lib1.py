...
def check_argv(argv):
	retval={}
	cmd_list=['addbr','create','start','config','del','stop','test','delbr','printdata','setbr','listbr','sshconfig','lxc-create','lxc-start','lxc-stop','lxc-delete']
	if len(argv) == 1:
		print_syntax()
	else:
    ...
    for i in list_of_lxc:
        ...
        if d1['lxc'][i]['type'] not in list_of_images:
            ...
            for p in ports:
                ...
                if 'ipv6' in d1['lxc'][i]['port'][p].keys():
                    net_cfg1.append(f"     inet6: {d1['lxc'][i]['port'][p]['ipv6']}")
